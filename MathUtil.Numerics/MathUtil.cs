...
public static class MathUtil
{
    ...
    {
        return new Vector2(
            RandomRange(rect.X, rect.X + rect.Width),
            RandomRange(rect.Y, rect.Y + rect.Height)
        );
    }
    ...
    public static Matrix4x4 RotateAround(this Matrix4x4 transform3D, Vector3 point, Vector3 axis, float angle)
    {
        return transform3D * Matrix4x4.CreateTranslation(-point) * Matrix4x4.CreateFromAxisAngle(axis, angle) * Matrix4x4.CreateTranslation(point);
    }
    ...
    {
        return Quaternion.CreateFromRotationMatrix(Matrix4x4.CreateWorld(Vector3.Zero, facingDirection, up));
    }
}
